...
export async function GET(req: NextRequest) {
  ...
  if (!invoiceId) return Response.json({ error: 'Falta invoice_id' }, { status: 400 })

  const { data: invoice, error } = await supabase
    .from('invoices')
    .select(`*, clients (*), organizations (id, tax_id, verifactu_emisor_id, verifactu_username, verifactu_api_key_encrypted), invoice_lines (*)`)
    .eq('id', invoiceId)
    .single()
  ...
  invoice_lines.forEach((line: any) => {
    ...
    desglose.CuotaRepercutida += vatAmount
  ...
  const facturaPayload: any = {
    ...
    ...(invoice.invoice_type === 'rectificativa' && invoice.original_invoice_number && {
    ...
    DescripcionOperacion: invoice.notes || 'Consulta médica',
    ...
    Destinatarios: [
      {
        ...
        ...(client.postal_code && { CodigoPostal: client.postal_code }),
        ...(client.city && { Municipio: client.city }),
        ...(client.province && { Provincia: client.province }),
        ...(client.country && { Pais: client.country }),
  ...
  // ✅ Obtener QR base64 si está presente
