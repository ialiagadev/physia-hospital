...
export async function POST(request: NextRequest, { params }: { params: { organizationId: string } }) {
  try {
    ...
    const { professionalId, serviceId, consultationId, date, startTime, endTime, duration, clientData, notes } = body

    if (isNaN(organizationId) || !professionalId || !date || !startTime || !endTime || !clientData) {
