...
export function RevenueByProfessional({ organizationId, period }: RevenueByProfessionalProps) {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        console.log("Fetching revenue by professional data:", {
        ...
        // Consultar líneas de factura con profesional asignado
        ...
        console.log("Invoice lines with professionals:", invoiceLines?.length || 0)

        // Agrupar por profesional
        const professionalRevenue: Record<string, { name: string; value: number }> = {}

        invoiceLines?.forEach((line) => {
          if (line.professional_id && line.professionals && typeof line.professionals === 'object' && 'name' in line.professionals) {
            const professionalName = line.professionals.name as string
            const amount = line.line_amount

            if (!professionalRevenue[professionalName]) {
              professionalRevenue[professionalName] = {
                name: professionalName,
                value: 0,
              }
            }

            professionalRevenue[professionalName].value += amount
          }
        })

        // Convertir a array para el gráfico
        const chartData = Object.values(professionalRevenue)
        console.log("Professional revenue data:", chartData)
  ...
  const renderCustomLabel = (props: PieLabelRenderProps) => {
    ...
    // Extraer las propiedades necesarias con comprobaciones de tipo
