...
export function WeeklyBillingModal({ isOpen, onClose, selectedDate }: WeeklyBillingModalProps) {
  ...
  const loadWeekAppointments = async () => {
    ...
    try {
      ...
      // Cargar citas individuales de toda la semana
      ...
      // Cargar actividades grupales de toda la semana
      ...
      await checkExistingInvoices(clientIds, weekStartStr, weekEndStr, clientsArray)
    } catch (error) {
      toast({
        title: "Error",
    ...
    try {
      ...
      for (let i = 0; i < validSelectedClients.length; i++) {
        ...
        try {
            ...
            .map((apt) => ({
              ...
              description:
                apt.type === "group_activity"
                  ? `Actividad Grupal: ${apt.activity_name} - ${apt.professional_name} (${format(new Date(apt.date), "dd/MM/yyyy", { locale: es })} ${apt.start_time}-${apt.end_time})`
                  : `${apt.service_name || "Servicio médico"} - ${apt.professional_name} (${format(new Date(apt.date), "dd/MM/yyyy", { locale: es })} ${apt.start_time}-${apt.end_time})`,
              quantity: 1,
          ...
          const additionalNotes = `Factura generada automáticamente para citas de la semana del ${format(weekStart, "dd/MM/yyyy", { locale: es })} al ${format(weekEnd, "dd/MM/yyyy", { locale: es })}`
            ...
            prevClients.map((client) => {
              if (client.client_id === clientId) {
                return {
                  ...
                  invoiceable_appointments: 0,
                  invoiced_appointments: client.appointments.length,
                  total_amount: 0,
                }
              }
              return client
  ...
  const checkExistingInvoices = async (
    ...
    weekStartStr: string,
    weekEndStr: string,
    clientsArray: ClientAppointmentData[],
  ) => {
        ...
        <div className="bg-purple-50 px-6 py-4 border-b border-purple-100">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-semibold text-gray-900">Facturación de la Semana</h2>
                <p className="text-sm text-gray-600">
                  {format(weekStart, "dd/MM/yyyy", { locale: es })} - {format(weekEnd, "dd/MM/yyyy", { locale: es })} •
                  Citas individuales + Actividades grupales
                </p>
              </div>
            </div>
          ...
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <div className="text-center">
                ...
                <p className="text-gray-600">Cargando todas las citas de la semana...</p>
              </div>
            </div>
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-600">No se encontraron citas ni actividades grupales para esta semana.</p>
            </div>
          ) : (
            <>
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                            <Link href={`/dashboard/clients/${client.client_id}`}>
                              <h3 className="font-medium text-gray-900 hover:text-purple-600 cursor-pointer transition-colors duration-200">
                                {client.client_name}
                              </h3>
                            </Link>
                            <Link href={`/dashboard/clients/${client.client_id}`}>
                          ...
                          {client.has_complete_data && client.invoiceable_appointments > 0 && (
                            <div className="mb-3 p-3 bg-purple-50 border border-purple-200 rounded">
                              <div className="flex items-center gap-2 mb-2">
                                ...
                                <Label className="text-sm font-medium text-purple-900">Método de Pago</Label>
                              </div>
                          ...
                          {/* Mostrar detalles de cada cita con su estado de facturación agrupadas por día */}
                          <div className="space-y-2 mb-3">
                            {weekDays.map((day) => {
                              const dayAppointments = client.appointments.filter(
                              ...
                              return (
                                <div key={format(day, "yyyy-MM-dd")} className="border border-gray-200 rounded p-2">
                          ...
                          <div className="flex justify-between items-center">
                            <div className="text-sm text-gray-600">
                              ...
                              {client.appointments.length !== 1 ? "es" : ""} en la semana
                            </div>
                            <div className="text-right">
